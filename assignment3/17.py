...
def TebuSearch(StringOfVar):
    ...
    while (limitChecker < limit and Heuristic(Clause, bestState) != (-1*k)):
        ...
        if len(CandidateOfBestState) == 0:
            print("No Futher Improvement possible stuck at local maxima " +
...
def VarNeighbourDescent(stringOfVar):
    ...
    if(checkValue == (-1*k)):
        ...
        print("Success! Number of states explored are " +
              str(totalStateVisited + 1))
    ...
    while(checkValue != (-1*k) and density <= n):
        ...
        if(checkValue == (-1*k)):
            ...
            print("Final State " + printState(stringOfVar))
            return True
        print("Present state " + printState(stringOfVar) +
              " with density " + str(density))
        ...
        if(density == n):
            ...
            print(
                "There is no solution for VND approach and number of Nodes explored are " + str(length_path))
            print("There is no solution for VND approach and number of states explored are " +
                  str(totalStateVisited))
...
def BeamSearch(stringOfVar, beta):
    ...
    while(not open.empty()):
        ...
        if(open.empty()):
            ...
            print("There is no solution for Beam search approach  and number of states explored are " +
                  str(totalStateVisited))
            return False
